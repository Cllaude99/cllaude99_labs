...
- Realtime - 게임 이벤트 동기화 (거래 확인, 결산 등)
...
- Yahoo Finance API - 2010-2024년 일별 OHLCV 데이터 수집
- Supabase Edge Function (`seed-stock-data`) - 데이터 수집 및 DB 적재 자동화
...
2월부터 11월까지는 실시간 차트로 주가 변동을 확인하며 투자를 진행합니다. 1거래일이 1초로 재생되어 1개월(약 22거래일)이 약 22초에 재생되며, 언제든지 매수/매도가 가능하고 포트폴리오의 실시간 수익률을 확인할 수 있습니다. 재생 속도는 1배속, 2배속, 5배속으로 조절 가능합니다.
...
Lightweight Charts 라이브러리를 사용하여 1년치 주가 데이터를 라인 차트로 표시합니다. 현재가와 등락률이 실시간으로 업데이트되며, 연도별 데이터를 한 번에 로드한 후 클라이언트에서 1일 1초 간격으로 순차 재생합니다. 1개월(약 22거래일)이 약 22초에 재생됩니다. 재생 속도는 1배속, 2배속, 5배속으로 조절 가능합니다.

### 블러 차트 미리보기

2년마다 발생하는 특별 이벤트로 블러 차트를 획득하면, 해당 연도의 3개월 구간 추세를 블러 처리된 형태로 미리 볼 수 있습니다. CSS blur 효과를 적용하여 전반적인 상승 또는 하락 추세는 파악할 수 있지만 세부 데이터는 숨겨져 있어, 장기 투자 전략 수립에 도움이 됩니다.
...
- Supabase 프로젝트 생성
- DB 스키마 설계 (stocks, stock_secrets, stock_daily_prices, game_sessions, portfolio_holdings, trade_history, yearly_settlements 등)
- `seed-stock-data` Edge Function으로 Yahoo Finance API에서 10개 종목 데이터 수집 및 적재
- Supabase Auth 설정
...
- 연도별 주가 데이터 조회 API - 1년치 데이터를 한 번에 반환, 클라이언트에서 재생
...
- 퀴즈 DB 구축 (`quizzes` 테이블 - 연도별 3문제, `hints` 테이블 - 연도별 레벨 1/2 힌트)
