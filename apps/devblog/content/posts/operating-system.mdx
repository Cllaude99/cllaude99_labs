...
초기 운영체제에서는 동기화 문제를 해결하기 위해 **Busy Waiting**(바쁜 대기) 방식을 사용했습니다. 이는 CPU를 계속 점유하면서 조건을 확인하는 비효율적인 방법이었습니다.
...
```mermaid
graph TB
    Parent1["부모 프로세스<br/>PID = 100"] -->|fork() 호출| Fork["fork() 시스템 콜"]
    Fork -->|복제| Child["자식 프로세스<br/>PID = 101"]
    Fork -->|반환값: 101| Parent2["부모 프로세스<br/>PID = 100"]

    Parent2 -.독립적 실행.-> ParentCode["부모 코드 실행"]
    Child -.독립적 실행.-> ChildCode["자식 코드 실행"]

    style Fork fill:#fff59d
    style Parent2 fill:#c5e1a5
    style Child fill:#ffcc80
```
...
이번 글에서는 운영체제의 핵심 개념들을 정리해보았습니다.
프로그램이 실행된다는 것의 의미부터 시작하여, 프로세스와 스레드의 차이, CPU 스케줄링, 메모리 관리까지 운영체제가 컴퓨터 자원을 어떻게 효율적으로 관리하는지 알아보았습니다.
학습 과정에서 멀티스레딩이 왜 효율적인지 이해하게 되었습니다. 스레드는 같은 프로세스 내에서 메모리를 공유하므로, 프로세스보다 생성 비용이 적고 Context Switching이 빠르다는 것을 알게 되었습니다.
