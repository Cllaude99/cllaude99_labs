...
sequenceDiagram
    ...
    participant A as 응용계층(HTTP)
    participant T as 전송계층(TCP)
    participant N as 네트워크계층(IP)
    participant D as 데이터링크계층(MAC)
    participant P as 물리계층(신호)

    U->>A: https://google.com 요청
    A->>T: HTTP GET 메시지 생성
    T->>N: TCP 세그먼트 생성
    Note over T,N: 포트 80 또는 443
    N->>D: IP 패킷 생성
    Note over N,D: 목적지 IP 주소
    D->>P: 프레임 생성
    Note over D,P: 목적지 MAC 주소
    P->>P: 전기 신호로 변환하여 전송
```
...
앞서 TCP의 여러 기능들(3-Way Handshake, 재전송, 흐름 제어, 혼잡 제어)을 살펴봤습니다. 이러한 기능들은 신뢰성을 보장하지만, 동시에 오버헤드를 발생시킨다는 특징이 있습니다.
그렇다면 신뢰성보다는 속도가 더 중요한 경우에는 어떻게 해야 할까요? 바로 이럴 때 UDP를 사용합니다.
UDP는 TCP와는 달리 연결을 수립하지 않고, 재전송도 하지 않으며, 순서도 보장하지 않습니다.
...
이러한 대칭키는 암호화/복호화 속도가 빠르지만, **키 교환 문제**가 있습니다.
그렇다면, 송신자와 수신자가 안전하게 같은 키를 공유하려면 어떻게 해야 할까요?

### 공개키 암호화

공개키 암호화는 대칭키의 키 교환 문제를 해결하기 위해 Diffie-Hellman에 의해 제안된 방식입니다.
공개키 방식은 공개키(Public Key)와 개인키(Private Key) 한 쌍을 사용하며 공개키로 암호화한 데이터는 개인키로만 복호화가 가능하며 개인키로 암호화한 데이터는 공개키로만 복호화가 가능합니다.
...
JWT(JSON Web Token) 같은 토큰을 사용하여 인증 정보를 클라이언트가 보관하게 할 수도 있습니다.
