...
평소 개발할 때 접근성이라고 하면 막연히 "시각 장애인을 위한 것"이라고만 생각하고 자세하게 알지는 못했어요.
그래서 접근성을 고려한다는 건 어떤 의미이고, 어떤 사람들을 위한 것일까? 에 대해 스스로 고민해봤고 아래와 같은 상황이 있을 수 있다는 것을 알게되었어요.
...
위의 상황들을 통해 알게된건,마치 엘리베이터가 휠체어 사용자만을 위한 것이 아니듯, 접근성도 특정 누군가만을 위한 것이 아니라는 거였어요.
오늘은 마우스로 편하게 쓰더라도 내일 손목을 다칠 수 있고, 밝은 햇빛 아래에서 화면을 봐야 할 수도 있고, 나이가 들며 시력이 나빠질 수도 있으니까요.
"내가 만든 UI를 대체 누가 쓰는 거지?" 라고 스스로에게 물었을 때, 마우스를 능숙하게 다루는 사람만, 화면이 잘 보이는 사람만 쓴다는 보장은 어디에도 없는 것 처럼요.
...
### CSR 환경에서도 시맨틱 태그가 중요할까?

여기까지 공부하다 보니 CSR 환경에서의 시맨틱 태그의 중요성에 대해 한 가지 의문이 들었어요.

> React 같은 CSR은 서버가 빈 HTML(`<div id="root">`)만 보내는데, 시맨틱 태그를 아무리 잘 써봤자 크롤러가 읽을 수 없는 거 아닌가?

이에 대해 추가학습을 해본 결과, 시맨틱 태그가 중요한 이유는 SEO 때문만이 아니었어요.
스크린 리더나 키보드 내비게이션 같은 보조 기술은 **브라우저가 렌더링한 DOM**을 기준으로 동작해요. 그래서 JavaScript가 그린 DOM이든 서버에서 내려준 DOM이든, 브라우저가 만드는 접근성 트리는 동일해요.
결론적으로는 CSR이라도 `nav`, `button`, `label` 같은 시맨틱 태그를 쓰면 보조 기술이 페이지 구조를 똑같이 파악할 수 있게 되는거죠.

SEO가 아예 안 되는 것도 아니에요.
구글 크롤러는 JavaScript를 실행하기 때문에 CSR의 시맨틱 태그도 인식할 수 있거든요.
비록 크롤링 예산 측면에서는 SSR이나 SSG가 더 유리하긴 하지만, 그건 시맨틱 태그의 문제가 아니라 렌더링 방식의 문제고요.

결국 렌더링 방식이 뭐든, 시맨틱 태그의 본질적 가치인 접근성은 달라지지 않는다는 것을 알게 되었어요.
...
시맨틱 태그를 올바르게 쓰면 SEO에도 긍정적인 영향을 주고, 결국 더 많은 사용자가 불편 없이 쓸 수 있는 UI로 이어지게 될 수 있는 것도 알게되었어요.
