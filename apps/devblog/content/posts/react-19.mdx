...
위의 예시처럼 로딩 상태 관리, 에러 처리, 상태 되돌리기까지 모두 비동기 처리를 할 때마다 반복되는 보일러플레이트였어요.
React 19는 이런 반복적인 패턴들을 프레임워크 레벨에서 지원하기 시작했어요.
...
React 19부터는 **async 함수**를 전달할 수 있게 되었어요. async transition이 진행되는 동안 React가 `isPending`을 `true`로 유지해주기 때문에, 로딩 상태를 직접 관리할 필요가 없어졌어요.

앞서 본 보일러플레이트를 `useTransition`으로 개선하면 아래와 같이 바꿀 수 있어요.
...
`useTransition`의 `startTransition`과 별도로, React에서 직접 import할 수 있는 `startTransition`도 있어요.
...
로딩 인디케이터를 보여줘야 한다면 `useTransition`을, pending 상태 추적 없이 transition만 시작하면 된다면 `startTransition`을 사용할 수 있어요.

## Actions - 비동기 처리의 통합 패턴

이렇게 `startTransition`이 async 함수를 지원하게 되면서, React 19에서는 이를 **Actions**라는 개념으로 정의하고 있어요.
즉, async 함수를 사용하는 transition을 "Action"이라고 불러요.
...
이후에 소개할 `useActionState`, `useFormStatus`, `useOptimistic` 모두 이 Actions 개념 위에서 동작해요.
...
또한 `<form>`의 `action` prop에 직접 전달할 수 있어서, 폼 제출 시 자동으로 리셋되어요.

### 서버 액션과의 연동

`useActionState`은 Next.js의 Server Actions와 함께 사용하면 더욱 편리한데요.
...
반면, `<form>`과 같은 레벨에서 호출하면 부모 `<form>`을 찾을 수 없어 동작하지 않아요.
...
서버에 요청을 보내고 응답이 올 때까지 기다린 후에야 하트가 채워진다면, 사용자 입장에서는 답답하게 느껴질 수 있어요.
...
`use`는 React 19에서 새롭게 추가된 API예요.

특이한 점이 있다면 기존의 훅들과 달리, **조건문이나 반복문 안에서도 호출할 수 있다는 것**이에요.
또, Suspense와 함께 사용하면 컴포넌트에서 Promise를 직접 읽을 수 있어요.
...
React 19부터는 함수 컴포넌트에서 `ref`를 일반 prop처럼 받을 수 있어요.
( 더 이상 `forwardRef`로 감싸지 않아도 돼요. )

기존에는 `forwardRef`로 감싸야 했어요.
...
이제는 `ref`를 일반 prop처럼 직접 받을 수 있어요.
...
참고로 `forwardRef`는 향후 deprecated 될 예정이에요.

### ref 콜백의 클린업 함수

ref 콜백에서 클린업 함수를 반환할 수 있게 되었어요.
( DOM 노드가 제거될 때 자동으로 호출돼요. )
...
기존에는 `<Context.Provider>`를 사용해야 했어요.
...
데이터 fetching과 함께 사용할 때는 Suspense와 결합하면 더 자연스러운 UX를 만들 수 있어요.
