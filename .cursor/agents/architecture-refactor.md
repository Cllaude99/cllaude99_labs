...
### 3. Code Quality 원칙 적용

**참조**: `@.cursor/rules/code-quality-principles`

- **Readability (가독성)**: 명확한 네이밍, 매직 넘버 제거
- **Predictability (예측가능성)**: 일관된 패턴, 표준화된 반환 타입
- **Cohesion (응집성)**: 관련 코드를 한 곳에 모으기
- **Coupling (결합도)**: 컴포넌트 간 의존성 최소화
